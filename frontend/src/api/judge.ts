...
const extractSimilarPrecedents = (
  ...
  return source.slice(0, 5).map((item, index) => {
    ...
    const caseId = String(row.case_id ?? row.caseId ?? row.similar_case_id ?? row.id ?? `precedent-${index + 1}`)
    const similarityScore = toNumber(row.similarity ?? row.score ?? row.similarity_score ?? row.similarityScore, 0)
